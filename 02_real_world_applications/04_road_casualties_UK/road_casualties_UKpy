...
dataset = 'collisions'
model = 'poisson'
dir = '/home/pradoe/code/01_PyMHSS/02_real_world_applications/04_road_casualties_UK/'

col_2020 = pd.read_csv(dir+'dft-road-casualty-statistics-collision-2020.csv', low_memory=False)
col_2021 = pd.read_csv(dir+'dft-road-casualty-statistics-collision-2021.csv', low_memory=False)
col_2022 = pd.read_csv(dir+'dft-road-casualty-statistics-collision-2022.csv', low_memory=False)
...
def run_MH_SS(x, y, theta_hat, V, taylor_order, nburn, npost, nthin, model, implementation, kappa=1.5, tuna_acc_rate = 0.24):
    ...
    if taylor_order == 0:
        ...
        method = MH_SS(y, x, V, x0 = theta_hat, model=model, control_variates=False, phi_function='original', taylor_order=taylor_order, chi=chi, nburn=nburn, npost=npost, kappa=kappa, nthin=nthin, implementation=implementation, calculate_ksd=False)    
    else:
