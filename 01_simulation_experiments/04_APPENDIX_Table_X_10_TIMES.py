...
colnames = np.array(['N','d', 'kappa', 'acc_rate', 'meanSJD', 'cpu_time', 'ESS', 'expected_B', 'acc_rate_ratio1', 'KSD'])

def run_tuna(y, x, theta_hat, V, taylor_order, npost, model, implementation, kappa=1.5, tuna_acc_rate = 0.24):
    N = x.shape[0]
    d = x.shape[1]
    nthin = 1000
    ...
    method = MH_SS(y, x, V, x0 = theta_hat, control_variates=False, phi_function='original', taylor_order=taylor_order, model=model, chi=chi, nburn=0, npost=npost, kappa=kappa, nthin=nthin, implementation=implementation, calculate_ksd=True)
    ...
    save_results = np.array([N, d, kappa, acc_rate, meanSJD, cpu_time, ESS, expected_B, acc_rate_ratio1, KSD])
...
def run_RWM(x, y, theta_hat, V, npost, model, implementation, kappa=2.4):
    ...
    d = x.shape[1]
    nthin = 10
    method = RWM(y, x, V, x0 = theta_hat, model=model, nburn=0, nthin=nthin, npost=npost, kappa = kappa, implementation=implementation, calculate_ksd=True)
    acc_rate = method.get('acc_rate')
    acc_rate_ratio1 = method.get('acc_rate_ratio1')
    meanSJD = method.get('meanSJD')
    cpu_time = method.get('cpu_time')
...
def run_MH_SS(y, x, V, theta_hat, taylor_order, npost, model, implementation, chi=0, kappa=1.5):
    ...
    nthin=10
    if taylor_order == 0:
        method = MH_SS(y, x, V=V, x0 = theta_hat, model=model, control_variates=False, taylor_order=taylor_order, chi=chi, nburn=0, nthin=nthin, npost=npost, kappa=kappa, implementation=implementation, calculate_ksd=True)    
    else:
        method = MH_SS(y, x, V=V, x0 = theta_hat, model=model, control_variates=True, taylor_order=taylor_order, chi=chi, nburn=0, nthin=nthin, npost=npost, kappa=kappa, implementation=implementation, calculate_ksd=True)
    acc_rate = method.get('acc_rate')
    acc_rate_ratio1 = method.get('acc_rate_ratio1')
    meanSJD = method.get('meanSJD')
    cpu_time = method.get('cpu_time')
...
def run_SMH(x, y, theta_hat, V, bound, taylor_order, npost, model, implementation):
    ...
    print(kappa)
    method = SMH(y, x, V, x0 = theta_hat, model=model, kappa=kappa, bound=bound, taylor_order=taylor_order, nburn=0, nthin=nthin, npost=npost,implementation=implementation, calculate_ksd=True)
    acc_rate = method.get('acc_rate')
    acc_rate_ratio1 = method.get('acc_rate_ratio1')
    meanSJD = method.get('meanSJD')
    cpu_time = method.get('cpu_time')
...
def run_methods(N, d, model, implementation, npost):
    ...
    for i in range(10):
        ...
        # ------------------------------------------------------------------------------------
        # tuna_024_results = run_tuna(y, x, theta_hat, V, taylor_order = 0, npost=npost, model=model, implementation=implementation, tuna_acc_rate = 0.24)
        # tuna_060_results = run_tuna(y, x, theta_hat, V, taylor_order = 0, npost=npost, model=model, implementation=implementation, tuna_acc_rate = 0.6)
...
def run_many_times(d, implementation, npost = 1000000, model = 'poisson'):
...
def get_results(N, model='poisson', implementation='loop'):

    set_d = np.array([10, 30, 50, 100])
    colnames = np.array(['N','d', 'kappa', 'acc_rate', 'meanSJD', 'cpu_time', 'ESS', 'expected_B', 'method', 'acc_rate_ratio1', 'KSD'])
    store_results = np.zeros((len(set_d), len(colnames)))
    store_results[:] = np.nan
    store_results = pd.DataFrame({'N':{},'d':{}, 'kappa':{}, 'acc_rate':{}, 'meanSJD':{}, 'cpu_time':{}, 'ESS':{}, 'expected_B':{}, 'method': {}, 'acc_rate_ratio1':{}, 'KSD': {}})
    ...
    store_results['method'] = pd.Categorical(store_results.method, categories=['MH-SS-1', 'MH-SS-2', 'SMH-1', 'SMH-2', 'SMH-1-NB', 'SMH-2-NB', 'RWM', 'Tuna', 'Tuna_024'])
...
table['N'] = table['N'].astype(str)
table['d'] = table['d'].astype(str)
table['KSD'] = table['KSD'].astype(float)
table['expected_B'] = table['expected_B'].astype(float)
table['ESS_per_second'] = table['ESS_per_second'].astype(float)
table['ESS_over_B'] = table['ESS_over_B'].astype(float)

table.groupby(['N', 'd', 'method']).agg({'expected_B': ['mean', 'std']}).reset_index()

table.groupby(['N', 'd', 'method']).agg({'ESS_per_second': ['mean', 'std']}).reset_index()

table.groupby(['N', 'd', 'method']).agg({'ESS_over_B': ['mean', 'std']}).reset_index()

table.groupby(['N', 'd', 'method']).agg({'KSD': ['mean', 'std']}).reset_index()
